...
class Produk extends CI_Controller {
	...
	{
		$data = array();
		$id_seller = $this->input->get('id_seller');
		...
		$query4 = $this->seller_model->info(array('id_seller' => $id_seller));
		
		if ($query4->num_rows() > 0)
		{
			$data['seller'] = $query4->row();
		}
	...
	function produk_get()
	{
        ...
        foreach ($query->result() as $row)
        {
            $action = '<a title="Edit" href="'.$this->config->item('link_admin_produk_update').'?id='.$row->id_product.'"><i class="fa fa-pencil h4"></i></a>&nbsp;
                        ...
                        <a title="Comments" href="'.$this->config->item('link_admin_comment').'?id_product='.$row->id_product.'"><i class="fa fa-comments h4 text-dark"></i></a>';
